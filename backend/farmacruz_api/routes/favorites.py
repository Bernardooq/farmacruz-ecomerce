...
def get_favorite(list_id: UUID, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    customer_id = get_customer_id(current_user)
    db_obj = crud_favorite.get_favorite_list(db, list_id, customer_id)
    if not db_obj:
